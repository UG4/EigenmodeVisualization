...
inline LGScene* getScene(unsigned idx)
{
	return getMainWindow()->get_scene(idx);
}

inline LGObject* createEmptyObject(const char* name, SceneObjectType sot, unsigned screen, unsigned idx)
{
    return getMainWindow()->create_empty_object(name, sot, screen, idx);
}

inline bool continue_oscillation()
{
    return getMainWindow()->m_oscillate;
}

inline unsigned numScenes()
{
    return getMainWindow()->m_scenes.size();
}

inline unsigned numObjects()
{
	return getMainWindow()->m_num_objects;
}

inline unsigned numIters()
{
	return getMainWindow()->m_num_iters;
}
